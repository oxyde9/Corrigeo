            ...
            $.get("http://api.ipapi.com/check?access_key=594c70b41fa4633610a7f80f14d28122&format=1").done(function (
              ...
              $.ajax({
                  ...
                  $.ajax({
                    ...
                    url: "api.php", // URL de Nominatim
                    type: 'get', // Requête de type GET
                    data: "action=write&name=" + ville + "&lat=" + lat + "&lon=" + lon +
"" // Données envoyées (q -> adresse complète, format -> format attendu pour la réponse, limit -> nombre de réponses attendu, polygon_svg -> fournit les données de polygone de la réponse en svg)
      ...
      var corrig = ` <?php $requete = "SELECT * FROM geoloc";
   $stmt = $db->query($requete);
   $result=$stmt->fetchall(PDO::FETCH_ASSOC);
   
   foreach ($result as $row){
       echo "<form><label for=\"coord\">".$row['name']."</label><br><input type=\"text\" name=\"lat\" value=".$row['lat']. "> | <input type=\"text\" value=".$row['lon']. " name=\"lon\"><a href=\"#\" id=\"mod\">Modifier</a><br></form>";
   }
  ?>`
     $("#corrig").html(corrig);
...
url: "api.php", // URL de Nominatim
type: 'get', // Requête de type GET
data: "action=modif&lat=" + lat + "&lon=" + lon +"" // Données envoyées (q -> adresse complète, format -> format attendu pour la réponse, limit -> nombre de réponses attendu, polygon_svg -> fournit les données de polygone de la réponse en svg)
})
